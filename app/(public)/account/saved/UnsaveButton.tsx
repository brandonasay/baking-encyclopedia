...
export default function UnsaveButton({ recipeId, onRemoved }: Props) {
  ...
  return (
    <button
      ...
      className="w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-red-50 hover:text-red-600 text-[#7A6A5E] transition-colors duration-150 disabled:opacity-60"
    >
      {status === 'loading' ? (
