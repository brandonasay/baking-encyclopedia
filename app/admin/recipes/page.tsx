  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#201D20]">Recipes</h1>
          <p className="text-sm text-[#6D5E6D] mt-0.5">{recipes.length} total</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-[#EBD2AD] overflow-hidden">
        {recipes.length === 0 ? (
          <div className="py-16 text-center text-[#6D5E6D]">
            <p>No recipes yet.</p>
            <Link href="/admin/recipes/new" className="text-[#C58930] hover:underline mt-2 inline-block text-sm">Create your first recipe</Link>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#EBD2AD] bg-[#FCFFEB]">
                  ...
                  <th className="text-left px-4 py-3 font-medium text-[#6D5E6D] hidden sm:table-cell">Difficulty</th>
              ...
              <tbody className="divide-y divide-[#EBD2AD]">
                {recipes.map((recipe) => (
                  <tr key={recipe.id} className="hover:bg-[#FCFFEB] transition-colors">
                    <td className="px-4 py-3">
                      <div>
                        <Link href={`/admin/recipes/${recipe.id}`} className="font-medium text-[#201D20] hover:text-[#C58930]">
                          {recipe.title}
                        </Link>
                        {recipe.featured && (
                          <span className="ml-2 text-xs px-1.5 py-0.5 bg-[#F5EAC8] text-[#C58930] rounded font-medium">Featured</span>
                        )}
                        <p className="text-xs text-[#6D5E6D] font-mono">{recipe.slug}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className="capitalize text-[#6D5E6D]">{difficultyLabel[recipe.difficulty]}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${recipe.published ? 'bg-[#EEF3EA] text-[#41622D]' : 'bg-[#FBF3DC] text-[#A87225]'}`}>
